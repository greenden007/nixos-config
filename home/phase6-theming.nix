...

{
  ...
  home.file.".config/matugen/config.toml".text = ''
    ...
    [templates.wallpaper_grid]
    input_path = "${config.home.homeDirectory}/.config/matugen/templates/wallpaper-grid.rasi"
    output_path = "${config.home.homeDirectory}/.local/share/rofi/themes/wallpaper-grid.rasi"
...
EOF
  ...
  # Generated by matugen after the first wallpaper selection. A writable static
  # fallback is seeded below for first boot so the picker can open before
  # matugen has ever run.
  home.file.".config/matugen/templates/wallpaper-grid.rasi".text = ''
    * {
      bg-col: {{colors.surface.default.hex}};
      bg-col-soft: {{colors.surface_container_low.default.hex}};
      bg-col-alt: {{colors.surface_container.default.hex}};
      border-col: {{colors.primary.default.hex}};
      selected-col: {{colors.primary.default.hex}};
      selected-fg: {{colors.on_primary.default.hex}};
      fg-col: {{colors.on_surface.default.hex}};
      muted-fg: {{colors.on_surface_variant.default.hex}};
      glow-col: {{colors.primary.default.hex}}4d;
    }
    window {
      width: 86%;
      height: 78%;
    ...
    inputbar {
      children: [prompt, entry];
      background-color: @bg-col-soft;
      border-radius: 16px;
      border: 1px;
      border-color: @bg-col-alt;
      padding: 10px 16px;
      spacing: 10px;
    }
    prompt { background-color: transparent; text-color: @muted-fg; }
    entry {
      background-color: transparent;
      text-color: @fg-col;
      placeholder: "Search wallpapers…";
      placeholder-color: @muted-fg;
    }
    listview {
      columns: 4;
      lines: 2;
...
EOF
    ...
    ensure_file "$HOME/.local/share/rofi/themes/wallpaper-grid.rasi" <<'EOF'
* {
  bg-col: #1e1e2e;
  bg-col-soft: #181825;
  bg-col-alt: #313244;
